...
const Authorization = (props) => {
  ...
  const searchUser = (text) => {
    if (text === "") {
      setUserFilter(authorizationList);
    }
    setUserFilter(
      authorizationList.filter((i) =>
        i.lastname.toLowerCase().includes(text.toLowerCase())
      )
    );
  };

  const deleteUserAuthorization = (id) => {
    axios
      .delete(`${baseURL}requests/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
        const requests = userFilter.filter((item) => item.id !== id);
        setUserFilter(requests);
        Toast.show({
          topOffset: 60,
          type: "success",
  ...
  const onRefresh = useCallback(() => {
    setRefreshing(true);
    setTimeout(() => {
      axios.get(`${baseURL}requests`).then((res) => {
              ...
              <FlatList
                ...
                renderItem={({ item, index }) => (
                  <AuthorizationListView
                    item={item}
                    index={index}
                    deleteUserAuthorization={deleteUserAuthorization}
                  />
                )}
