      ...
      <tbody>
        ...
        <tr class="flex flex-col ">
         ${order.orderItems
          ...
          <td class="flex-1">${orderItem.product.productName}</td>
          <td class="min-w-[44px]">₱${orderItem.product.price.toFixed(
            2
          )}</td>
        </tr>
        `
    ...
    const fetchOrderItems = async () => {
      try {
        const response = await axios.get(
          `${baseURL}orders/get/userOrders/${order._id}`
        );

        // Update state with the request items from the response
        setOrderItems(response.data[0]?.orderItems || []); // Adjust the array index based on your response structure
      } catch (error) {
